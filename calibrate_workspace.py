...
def draw_marker_axes(color_image, corners_list, ids_flat, marker_size_mm,
                     camera_matrix=None, dist_coeffs=None):
    """
    Draw x, y, z axes at corner 0 of each detected ArUco marker.

    Uses cv2.solvePnP per marker and cv2.drawFrameAxes for visualization.
    If no camera intrinsics are provided, estimates them from the image size.
    ...
    Args:
        ...
        camera_matrix: 3x3 intrinsic matrix (optional).
        dist_coeffs: Distortion coefficients (optional).

    Returns:
        np.ndarray: Annotated BGR image with axes drawn.
    """
    annotated = color_image.copy()
    if len(corners_list) == 0:
        return annotated
    ...
    # Object points with corner-0 at the origin
    ...
    for corners, marker_id in zip(corners_list, ids_flat):
        ...
        if success:
            cv2.drawFrameAxes(
                ...
                rvec, tvec, axis_length, thickness=2,
            )
            # Label the marker ID near corner 0
            c0 = tuple(img_pts[0].astype(int))
            cv2.putText(
                annotated, f"ID {marker_id}",
                (c0[0] + 5, c0[1] - 10),
...
def main():
    ...
    # --- Save marker axes snapshot ---
    marker_size_mm = config.get("marker_size_mm", 50)
    axes_image = draw_marker_axes(
        color_image, corners_list, ids_flat, marker_size_mm
    )
    axes_path = os.path.join(os.path.dirname(args.output), "marker_axes.png")
    os.makedirs(os.path.dirname(axes_path), exist_ok=True)
    cv2.imwrite(axes_path, axes_image)
    print(f"Marker axes snapshot saved to: {axes_path}")

    # --- Compute pairwise distances between marker corner-0 points ---
    positions = compute_marker_distances(
        corners_list, ids_flat, marker_size_mm, color_image.shape
    )
    if len(positions) >= 2:
        sorted_ids = sorted(positions.keys())
        print(f"\nMarker corner-0 positions (camera X, Y, Z in mm):")
        for mid in sorted_ids:
            x, y, z = positions[mid]
            print(f"  ID {mid}: X={x:.1f}, Y={y:.1f}, Z={z:.1f}")

        print(f"\nPairwise X-Y plane distances between corner-0 points (mm):")
        for i in range(len(sorted_ids)):
            for j in range(i + 1, len(sorted_ids)):
                id_a, id_b = sorted_ids[i], sorted_ids[j]
                xa, ya, _ = positions[id_a]
                xb, yb, _ = positions[id_b]
                dist = np.sqrt((xb - xa) ** 2 + (yb - ya) ** 2)
                print(f"  ID {id_a} <-> ID {id_b}: {dist:.1f} mm")
    else:
        print("[WARNING] Need at least 2 markers to compute distances.")
